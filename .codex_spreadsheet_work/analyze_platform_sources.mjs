...
const headerIndex = (values, required = []) => {
  ...
  values.slice(0, 50).forEach((row, index) => {
    ...
    const score =
      row.filter((v) => norm(v)).length + required.filter((label) => labels.includes(label)).length * 1000;
...
async function analyzeShopee() {
  ...
  for (const row of rows) {
    ...
    if (!orders.has(id))
      orders.set(id, {
        date: dateKey(row[c("Ngày đặt hàng")]),
        status: norm(row[c("Trạng Thái Đơn Hàng")]),
        buyerPaid: num(row[c("Tổng số tiền Người mua thanh toán")]),
        orderValue: num(row[c("Tổng giá trị đơn hàng (VND)")]),
        rows: 0,
        quantity: 0,
      });
  ...
  const isSuccessful = (status) =>
    ...
    status.startsWith("Người mua xác nhận đã nhận được hàng");
  return {
    headerRow: hi + 1,
    headers,
    ...
    statuses: Object.fromEntries([
      ...unique.reduce(
        (m, o) => m.set(o.status || "(trống)", (m.get(o.status || "(trống)") ?? 0) + 1),
        new Map(),
      ),
    ]),
    ...
    orderValueCompleted: unique
      .filter((o) => o.status === "Hoàn thành")
      .reduce((s, o) => s + o.orderValue, 0),
    daily: Object.fromEntries(
      [
        ...unique.reduce((m, o) => {
          const d = m.get(o.date) ?? {
            orders: 0,
            successful: 0,
            cancelled: 0,
            pending: 0,
...
async function analyzeTikTok() {
  ...
  return {
    ...
    daily: rows.map((row) => ({
      ...
      pageViews: num(row[8]),
      visitors: num(row[9]),
      conversionRate: num(row[10]),
      aov: num(row[15]),
    })),
    totals: {
      gmv: rows.reduce((s, r) => s + num(r[1]), 0),
      orders: rows.reduce((s, r) => s + num(r[2]), 0),
      revenue: rows.reduce((s, r) => s + num(r[7]), 0),
      refund: rows.reduce((s, r) => s + num(r[5]), 0),
    },
  };
}
...
async function analyzeFacebook() {
  ...
  return {
    ...
    revenue: unique
      .filter((row) => norm(row[c("Trạng thái đơn hàng")]) !== "Đã hủy")
      .reduce((s, row) => s + num(row[c("Khách phải trả")]), 0),
    paid: unique
      .filter((row) => norm(row[c("Trạng thái đơn hàng")]) !== "Đã hủy")
      .reduce((s, row) => s + num(row[c("Khách đã trả")]), 0),
    daily: Object.fromEntries(
      [
        ...unique.reduce((m, row) => {
...
const result =
  requested === "shopee"
    ? { shopee: await analyzeShopee() }
    : requested === "tiktok"
      ? { tiktok: await analyzeTikTok() }
      : requested === "facebook"
        ? { facebook: await analyzeFacebook() }
