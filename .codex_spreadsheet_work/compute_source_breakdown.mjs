...
async function orderData(path) {
  ...
  const orders = [...byId.values()]
    .map((r) => ({
      id: norm(r[c("Mã ĐH")]),
      date: dateKey(r[c("Ngày chứng từ")]),
      source: norm(r[c("Nguồn")]) || "(trống)",
      status: norm(r[c("Trạng thái đơn hàng")]),
      gross: num(r[c("Khách phải trả")]),
      paid: num(r[c("Khách đã trả")]),
    }))
  ...
  const bySource = Object.fromEntries(
    [
      ...orders.reduce((m, o) => {
        const a = m.get(o.source) || [];
        a.push(o);
        m.set(o.source, a);
        return m;
      }, new Map()),
    ]
      .map(([k, a]) => [k, summarize(a)])
...
async function tiktokData(path) {
  ...
  const rows = values
    ...
    .map((r) => {
      ...
      return {
        date: dateKey(r[c("Ngày")]),
        gmv,
        orders: num(r[c("Đơn hàng")]),
        liveCreator,
        liveConnected,
        live,
        videoCreator,
  ...
  const total = Object.fromEntries(
    [
      ...
      "video",
      "productCard",
    ].map((k) => [k, rows.reduce((s, r) => s + r[k], 0)]),
  );
  return { headers: h, rows, total };
}

const [long, facebook, tiktok] = await Promise.all([
  orderData(paths.long),
  orderData(paths.facebook),
  tiktokData(paths.tiktok),
]);
...
console.log(
  JSON.stringify(
    { long, facebook, overlap: { count: overlap.length, sample: overlap.slice(0, 20) }, tiktok },
    null,
    2,
  ),
);
