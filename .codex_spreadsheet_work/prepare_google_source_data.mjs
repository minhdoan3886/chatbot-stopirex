...
const facebookOrders = [...byId.values()]
  .map((r) => {
    const gross = num(r[fc("Khách phải trả")]);
    const paid = num(r[fc("Khách đã trả")]);
    return [
      dateKey(r[fc("Ngày chứng từ")]),
      norm(r[fc("Mã ĐH")]),
      norm(r[fc("Nguồn")]) || "(trống)",
      norm(r[fc("Trạng thái đơn hàng")]),
      gross,
      paid,
      gross - paid,
      norm(r[fc("Tên khách hàng")]),
      norm(r[fc("Điện thoại KH")]),
    ];
  })
  .filter((r) => r[0] >= "2026-08-01" && r[0] <= "2026-08-10")
...
const tiktokDaily = tv
  .slice(thi + 1)
  .map((r) => [
    dateKey(r[tc("Ngày")]),
    num(r[tc("GMV")]),
    num(r[tc("Đơn hàng")]),
    num(r[tc("Tổng doanh thu")]),
    num(r[tc("Hoàn tiền")]),
    num(r[tc("GMV nhờ buổi LIVE của nhà sáng tạo")]),
    num(r[tc("GMV nhờ buổi LIVE của tài khoản kết nối")]),
    num(r[tc("GMV đến từ video liên kết")]),
    num(r[tc("GMV nhờ video của tài khoản kết nối")]),
  ])
  .filter((r) => r[0] >= "2026-08-01" && r[0] <= "2026-08-10")
