...
export class MetaOAuthService {
  ...
  async authorizedPages(code: string): Promise<MetaOAuthPage[]> {
    ...
    const shortUserToken = await this.exchangeCode(code);
    const longUserToken = await this.exchangeLongLivedUserToken(shortUserToken);
    return this.listPages(longUserToken);
  }
  ...
  private signature(encodedPayload: string): Buffer {
    return createHmac("sha256", this.options.appSecret)
      .update(`${encodedPayload}.${this.options.redirectUri}`, "utf8")
      .digest();
  }
  ...
  private async exchangeCode(code: string): Promise<string> {
    const url = new URL(
      `https://graph.facebook.com/${this.options.graphVersion}/oauth/access_token`,
    );
    url.searchParams.set("client_id", this.options.appId);
  ...
  private async exchangeLongLivedUserToken(shortUserToken: string): Promise<string> {
    const url = new URL(
      `https://graph.facebook.com/${this.options.graphVersion}/oauth/access_token`,
    );
  ...
  private async readAccessToken(url: URL, errorCode: string): Promise<string> {
  ...
  private async listPages(userToken: string): Promise<MetaOAuthPage[]> {
    ...
    while (next && pageCount < 10) {
      ...
      if (!response.ok || !Array.isArray(payload.data)) {
        throw new Error("meta_oauth_pages_fetch_failed");
      }
