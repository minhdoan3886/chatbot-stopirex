...
export class MetaPageManagementService {
  ...
  async connectAuthorizedPages(
    ...
    for (const item of pages) {
      const profile = await this.readPageProfile(item.accessToken);
      if (profile.id !== item.id) throw new Error("meta_oauth_page_token_mismatch");
      await this.subscribePage(profile.id, item.accessToken);
      ...
      const id = await this.options.store.upsertFacebookPageConnection({
        ...
        encryptedAccessToken: this.options.vault.encrypt(item.accessToken),
      });
      this.cache.delete(id);
      const page = (await this.list()).find((candidate) => candidate.id === id);
