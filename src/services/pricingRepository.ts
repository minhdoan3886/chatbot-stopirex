...
export class InMemoryPricingRepository implements PricingRepository {
  ...
  activePrices(input: { tenantId: TenantId; channel: Channel; at: Date }): PriceRecord[] {
    return this.records
      .filter(
        (record) =>
...
export class PgPricingRepository implements PricingRepository {
  ...
  async activePrices(input: { tenantId: TenantId; channel: Channel; at: Date }): Promise<PriceRecord[]> {
    ...
    return result.rows.map((row) => ({
      ...
      ...(row.offer_version ? { offerVersion: String(row.offer_version) } : {}),
      ...(row.approved_by ? { approvedBy: String(row.approved_by) } : {}),
    }));
  }

  async catalog(input: { tenantId: TenantId; channel: Channel; at: Date }): Promise<ProductCatalog> {
